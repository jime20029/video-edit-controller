...
def verify_edited_video(edited_video, allowed_list):
    ...
    edited_video_frozenset = frozenset(edited_video)
    return edited_video_frozenset in allowed_list

# 東北大学側システムに問い合わせて塊ごとのスコア取得
def get_segment_scores(original_video):
    # 東北大学側システムに問い合わせする処理を追加
    return {frozenset({'a', 'b'}): 0.2, frozenset({'a', 'b', 'e'}): 0.6, frozenset({'a', 'e'}): 0.9}
...
def main():
    ...
    print('Setup Completed. Start Distributing.')
